...
class Trait(models.Model):
    ...
    mood_impact = models.CharField(
        ...
        blank=True,
        help_text=(
              "How this trait influences mood "
              "(e.g. +Playful, -Calm)"
        )
    )
...
class Dinosaur(models.Model):
    ...
    def __str__(self):
        return (
            f"🦕 {self.name} "
            f"({self.stage}, {self.mood})"
        )

    def get_sprite(self):
        """
        Return the relative static path for the dino's
        current stage sprite.
        """
...
class Trade(models.Model):
    ...
    sender_egg = models.ForeignKey(
        Egg, null=True, blank=True, on_delete=models.SET_NULL,
        related_name='sent_trades'
    )
    sender_dinosaur = models.ForeignKey(
        Dinosaur, null=True, blank=True, on_delete=models.SET_NULL,
        related_name='sent_dino_trades'
    )
    receiver_egg = models.ForeignKey(
        Egg, null=True, blank=True, on_delete=models.SET_NULL,
        related_name='received_trades'
    )
    receiver_dinosaur = models.ForeignKey(
        Dinosaur, null=True, blank=True, on_delete=models.SET_NULL,
        related_name='received_dino_trades'
    )
    ...
    def clean(self):
        ...
        if sum([item is not None for item in sender_items]) != 1:
            raise ValidationError(
                'Sender must offer exactly one item '
                '(egg or dinosaur).'
            )
        if sum([item is not None for item in receiver_items]) != 1:
            raise ValidationError(
                'Receiver must offer exactly one item '
                '(egg or dinosaur).'
            )
